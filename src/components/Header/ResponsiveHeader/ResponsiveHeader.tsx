...
const ResponsiveHeader: FC<IProps> = ({ setOpen }) => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="lg:hidden absolute top-[76px] left-0 w-full bg-[#363636] rounded-b-2xl shadow-[0_4px_12px_rgba(0,0,0,0.12)] z-40"
      ...
      >
        <div className="flex justify-end p-4">
          <button
            className="text-[#AAAAAA] text-2xl"
        ...
        <nav className="flex flex-col gap-4 p-4 text-[#AAAAAA]">
          {[
            { href: "/reserve", label: "رزرو سریع" },
            { href: "/mortgage-rent", label: "رهن و اجاره" },
            { href: "/about", label: "درباره دلتا" },
            { href: "/blogs", label: "مقالات ما" },
            { href: "/contact-us", label: "تماس با ما" },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`font-[500] text-[16px] flex flex-row items-center gap-2 transition-colors ${
                pathName === item.href ? "text-[#8CFF45]" : ""
              }`}
              onClick={() => setOpen(false)}
            >
              {item.label}
              {item.href !== "/about" &&
                ...
                item.href !== "/contact-us" && (
                  ...
                  >
                    <path
                      d="M1 1L5 5L9 1"
                      stroke={pathName === item.href ? "#8CFF45" : "#AAAAAA"}
                      strokeWidth="2"
                    />
                  </svg>
                )}
