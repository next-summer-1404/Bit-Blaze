...
const Header = () => {
  ...
  return (
    <header className="bg-[#363636] mx-auto relative translate-y-6 max-w-[97%] w-full flex flex-row justify-between p-2 rounded-2xl shadow-[inset_0_4px_8px_rgba(255,255,255,0.12),0_4px_12px_rgba(0,0,0,0.12)] z-[999]">
      <div className="flex flex-row gap-6 items-center">
        <div className="flex flex-row gap-5 items-center">
          <div className="w-24 h-9 bg-[#4A4A4A] rounded-xl"></div>
          <div className="w-1 h-5 bg-[#4A4A4A] rounded-xl"></div>
        </div>
        <nav className="flex flex-row gap-10 text-[#AAAAAA] items-center">
          <div className="relative">
            <Link href={'/'} className={`font-[500] text-[16px] h-fit text-[${pathName === '/' ? "#8CFF45" : "#AAAAAA"}] flex flex-row items-center gap-2 transition-colors`}>
              رزرو سریع
              <svg width="10" height="7" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1L5 5L9 1" stroke={pathName === '/' ? "#8CFF45" : "#AAAAAA"} strokeWidth="2" />
              </svg>

            </Link>
            {pathName === '/' && (
              <span className="absolute w-full m-0 bottom-[-16px] rounded-t-[12px] h-1 bg-[#8CFF45]"></span>
            )}
          </div>
          <div className="relative">
            <Link href={'/mortgage-rent'} className={`font-[500] text-[16px] h-fit text-[${pathName === '/mortgage-rent' ? "#8CFF45" : "#AAAAAA"}]   flex flex-row items-center gap-2 transition-colors`}>
              رهن و اجاره
              <svg width="10" height="7" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1L5 5L9 1" stroke={pathName === '/mortgage-rent' ? "#8CFF45" : "#AAAAAA"} strokeWidth="2" />
              </svg>
          ...
          <div className="relative">
            <Link href={'/about'} className={`font-[500] text-[16px] h-fit text-[${pathName === '/about' ? "#8CFF45" : "#AAAAAA"}] flex flex-row items-center gap-2 transition-colors`}>
              درباره دلتا
            </Link>
            {pathName === '/about' && (
              <span className="absolute w-full m-0 bottom-[-16px] rounded-t-[12px] h-1 bg-[#8CFF45]"></span>
          ...
          <div className="relative">
            <Link href={'/blogs'} className={`font-[500] text-[16px] h-fit text-[${pathName === '/blogs' ? "#8CFF45" : "#AAAAAA"}] flex flex-row items-center gap-2 transition-colors`}>
              مقالات ما
            </Link>
            {pathName === '/blogs' && (
              <span className="absolute w-full m-0 bottom-[-16px] rounded-t-[12px] h-1 bg-[#8CFF45]"></span>
          ...
          <div className="relative">
            <Link href={'/contact-us'} className={`font-[500] text-[16px] h-fit text-[${pathName === '/contact-us' ? "#8CFF45" : "#AAAAAA"}] flex flex-row items-center gap-2 transition-colors`}>
              تماس با ما
            </Link>
            {pathName === '/contact-us' && (
              <span className="absolute w-full m-0 bottom-[-16px] rounded-t-[12px] h-1 bg-[#8CFF45]"></span>
